...
function removeFirstOccurrences(str, value) {
  ...
  if (index !== -1) {
    ...
    result = str.substring(0, index) + str.substring(index + value.length);
  }
...
function sumOfCodes(str) {
  // return str.split('').reduce((acc, item) => (acc += item.charCodeAt()), 0);
...
function formatTime(minutes = 0, seconds = 0) {
  const min = minutes.toString().padStart(2, '0').concat(':');
...
function countVowels(str) {
  // throw new Error('Not implemented');
  const result = str.replace(/[^aeiouy]/gi, '');
...
function isPalindrome(str) {
  // throw new Error('Not implemented');
  const result = str.replace(/[\s,!?:;-]/gi, '').toLowerCase();
...
function findLongestWord(sentence) {
  // throw new Error('Not implemented');
  // Решение конечно дурацкое, но лень оптимизировать( reduce или циклом с переменной
  const sentArr = sentence.split(' ');
  sentArr.sort((a, b) => b.length - a.length);
  return sentArr[0];
}
...
function extractNameFromTemplate(value) {
  // Решение, конечно, дурацкое, но очень уж поиграться хотелось
  const d = 'Hello, |!';
  return value.replace(new RegExp(d, 'g'), '');
  // const punctMark = '!';
  // const d = 'Hello, ';
  // return value.replace(new RegExp(d + '|' + punctMark, 'g'), '');
  // return value.replace(new RegExp(d, 'g'), '');
}
...
function encodeToRot13(str) {
  const latinABC = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz!?.,';
  const rotABC = 'NOPQRSTUVWXYZABCDEFGHIJKLM nopqrstuvwxyzabcdefghijklm!?.,';
  let rotStr = '';
  for (let i = 0; i < str.length; i += 1) {
    const index = latinABC.indexOf(str[i]);
    rotStr += rotABC[index];
  }
  return rotStr;
}
...
function getCardId(value) {
  ...
  // Решение со строкой
  // Делаю строку заменяя 10 на 1, чтобы было одинаковое смещение, потом на входе надо убрать 0 из 10
  const str =
    'A♣2♣3♣4♣5♣6♣7♣8♣9♣1♣J♣Q♣K♣A♦2♦3♦4♦5♦6♦7♦8♦9♦1♦J♦Q♦K♦A♥2♥3♥4♥5♥6♥7♥8♥9♥1♥J♥Q♥K♥A♠2♠3♠4♠5♠6♠7♠8♠9♠1♠J♠Q♠K♠';
  const valueSearch = value.replace(/[0]/gi, '');
  return str.indexOf(valueSearch) / 2;
}
